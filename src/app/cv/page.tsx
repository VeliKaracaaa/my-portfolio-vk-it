  ...
  return (
    <main className="min-h-screen bg-[#FBFBFE] text-[#1E293B] font-sans p-4 md:p-12 relative">
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-x-10 gap-y-6 lg:gap-y-10">
        {/* BOUTON RETOUR ET TÉLÉCHARGEMENT */}
        <div className="print:hidden lg:col-start-2 lg:row-start-1 flex flex-col sm:flex-row gap-4 h-fit">
          <Link
            href="/"
            className="flex-1 flex items-center justify-center gap-2 p-4 bg-white rounded-3xl border border-slate-200 font-bold text-sm hover:bg-slate-50 transition-colors shadow-sm text-blue-600"
          >
            <HomeIcon size={18} /> Retour à l&apos;accueil
          </Link>
          <button
            onClick={() => window.print()}
            className="flex-1 flex items-center justify-center gap-2 p-4 bg-blue-600 text-white rounded-3xl border border-blue-600 font-bold text-sm hover:bg-blue-700 transition-colors shadow-sm"
          >
